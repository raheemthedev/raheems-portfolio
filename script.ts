...
const shaderCanvas = document.querySelector<HTMLCanvasElement>(".project-shader");
...
const shells = [...track.querySelectorAll<HTMLElement>(".project-shell")];
const reducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)");
const siteLoader = document.querySelector<HTMLElement>(".site-loader");
const loaderWord = siteLoader?.querySelector<HTMLElement>(".site-loader__word") ?? null;
const loaderPieces = [...document.querySelectorAll<HTMLElement>("[data-loader-piece]")];
const cornerTargets = [
  document.querySelector<HTMLElement>(".corner-mark--top-left"),
  document.querySelector<HTMLElement>(".corner-mark--top-right"),
  document.querySelector<HTMLElement>(".corner-mark--bottom-left"),
...
const heroContactCta = document.querySelector<HTMLElement>(".hero-cta");
const heroCtaIdleLabel = heroContactCta?.querySelector<HTMLElement>(".hero-cta__label--idle") ?? null;
const heroCtaEmailLabel = heroContactCta?.querySelector<HTMLElement>(".hero-cta__label--email") ?? null;

const splitCtaLabel = (label: HTMLElement) => {
...
if (heroContactCta && heroCtaIdleLabel && heroCtaEmailLabel) {
  ...
  const ctaAnimations = new Set<Animation>();
  let ctaExpanded = false;

  const characterOffset = (index: number, revealing: boolean) => {
  ...
  const setCtaExpanded = (expanded: boolean) => {
    if (expanded === ctaExpanded) return;
...
const runSiteLoader = async () => {
  if (window.location.hash === "#contact") {
    ...
    requestAnimationFrame(() => {
      const contactTarget = document.querySelector<HTMLElement>("#contact");
  ...
  const panelTop = siteLoader?.querySelector<HTMLElement>(".site-loader__panel--top");
  const panelRight = siteLoader?.querySelector<HTMLElement>(".site-loader__panel--right");
  const panelBottom = siteLoader?.querySelector<HTMLElement>(".site-loader__panel--bottom");
  ...
  const phaseTwoPanels = [
    panelTop.animate(
      [{ height: `${windowFrame.top}px` }, { height: "0px" }],
      { duration: phaseTwoDuration, easing: phaseTwoEasing, fill: "forwards" },
    ),
    panelBottom.animate(
      [{ height: `${viewportHeight - windowFrame.bottom}px` }, { height: "0px" }],
      { duration: phaseTwoDuration, easing: phaseTwoEasing, fill: "forwards" },
    ),
    panelLeft.animate(
      [
...
const beginMotion = (offsetDelta: number, now = performance.now()) => {
  if (reducedMotion.matches || Math.abs(offsetDelta) < 0.01) return;
...
const revealMetadata = () => {
  if (reducedMotion.matches || isDragging || !metadataHidden) return;
...
const createShaderRenderer = (canvas: HTMLCanvasElement): ShaderRenderer | null => {
  ...
  const render: ShaderRenderer["render"] = (
    time,
    velocity,
    railX,
    stride,
...
const measureRail = () => {
  const firstTile = track.querySelector<HTMLElement>(".project-shell");
  if (!firstTile) return;
...
const renderFramePhysics = (deltaTime: number) => {
...
const renderRail = (timestamp: number) => {
  animationFrame = null;
  ...
  const gyroMoving =
    Math.abs(gyroTargetX - gyroX) > 0.001 ||
...
const addVelocity = (distance: number, strength: number) => {
...
const updateGyroFromPointer = (event: PointerEvent) => {
  const bounds = landing.getBoundingClientRect();
  const isInsideLanding =
...
const featuredProjects = [...document.querySelectorAll<HTMLElement>(".featured-project")];
